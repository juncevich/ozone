...
public class CSMMetrics {
  ...
  private final MetricsRegistry registry = new MetricsRegistry(CSMMetrics.class.getSimpleName());
  ...
  private @Metric ReadWriteLockMutableRate transactionLatencyMs;
  private final EnumMap<Type, ReadWriteLockMutableRate> opsLatencyMs;
  private final EnumMap<Type, ReadWriteLockMutableRate> opsQueueingDelay;
  ...
  private @Metric ReadWriteLockMutableRate applyTransactionNs;
  private @Metric ReadWriteLockMutableRate writeStateMachineDataNs;
  private @Metric ReadWriteLockMutableRate writeStateMachineQueueingLatencyNs;
  private @Metric ReadWriteLockMutableRate untilApplyTransactionNs;
  private @Metric ReadWriteLockMutableRate startTransactionCompleteNs;
...
//    this.registry = new MetricsRegistry(CSMMetrics.class.getSimpleName());
    Map<String, MutableMetric> customMetrics = new HashMap<>();
    for (ContainerProtos.Type type : ContainerProtos.Type.values()) {
      ReadWriteLockMutableRate value1 = new ReadWriteLockMutableRate(type.toString() + "Ms", type + " op");
      opsLatencyMs.put(type, value1);
      customMetrics.put(type + "Ms", value1);
      ReadWriteLockMutableRate value2 = new ReadWriteLockMutableRate(
          "queueingDelay" + type + "Ns", type + " op");
      opsQueueingDelay.put(type, value2);
      customMetrics.put("queueingDelay" + type + "Ns", value2);
    }
    OzoneMetricsSystem.registerNewRates(registry, customMetrics);
  }
  ...
  ReadWriteLockMutableRate getApplyTransactionLatencyNs() {
